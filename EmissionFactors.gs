...
function buildEmissionFactorPayload_(data, existing) {
  ...
  return {
    ...
    co2_unit: data.co2_unit || 'kg CO2/' + (data.unit || 'unit'),
    fossil_ch4_unit: data.fossil_ch4_unit || 'kg fossil CH4/' + (data.unit || 'unit'),
    ch4_unit: data.ch4_unit || 'kg CH4/' + (data.unit || 'unit'),
    n2o_unit: data.n2o_unit || 'kg N2O/' + (data.unit || 'unit'),
